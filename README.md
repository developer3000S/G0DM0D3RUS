...
G0DM0D3RUS — полностью открытый, конфиденциальный многомодельный чат-интерфейс, который расширяет возможности пост-тренировочного слоя — для red teaming, исследований когниции и свободного взаимодействия с ИИ. Создан для хакеров, философов и системных энтузиастов.
...
## 📡 API-Сервер и Ротация Ключей

G0DM0D3 включает в себя опциональный высокопроизводительный бэкенд API-сервер (на Node.js/TypeScript), который развертывается локально или в Docker. Сервер предоставляет полную OpenAI-совместимость и управляет многомодельными гонками.

### 🔑 Динамическая Ротация API Ключей (Failover & Load Balancing)
Для обхода жестких лимитов провайдеров (`HTTP 429 Too Many Requests`), борьбы с ошибками сети или исчерпания средств на балансе, в API-сервер встроен **автоматический пул ротации ключей**.

Вы можете указать неограниченное количество ключей для любого провайдера в вашем `.env` файле, и сервер настроит ротацию сам:
```env
# Ключи для OpenRouter (с автоматической ротацией)
OPENROUTER_API_KEY=sk-or-v1-primary...
OPENROUTER_API_KEY1=sk-or-v1-key1...
OPENROUTER_API_KEY2=sk-or-v1-key2...

# Ключи для Mistral
MISTRAL_API_KEY1=6UBrR2T0P79...
MISTRAL_API_KEY2=8rxR7VdecIy...
...
**Особенности работы пула ротации:**
1. **Балансировка**: При каждом запросе сервер выбирает случайный стартовый ключ, равномерно распределяя нагрузку.
2. **Отказоустойчивость**: Если ключ исчерпал лимиты (ошибка 429/402/403/сбой сети), сервер автоматически и мгновенно переключается на следующий рабочий ключ в списке и пробует снова (до 3 попыток на запрос).
3. **Безопасность**: Логи сервера маскируют секреты, отображая только последние 4 символа используемого ключа.
4. **Docker-совместимость**: Скрипт `./start.sh` автоматически считывает все номерные ключи из вашей системы и пробрасывает их внутрь Docker-контейнера.

Подробнее о конфигурации и роутинге см. в [API.md](API.md).
