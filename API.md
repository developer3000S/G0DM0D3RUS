...
| `OPENROUTER_API_KEY[1-N]` | Список ключей OpenRouter для автоматической ротации | _(none)_ |
| `MISTRAL_API_KEY[1-N]` | Список ключей Mistral для автоматической ротации | _(none)_ |
| `GROQ_API_KEY[1-N]` | Список ключей Groq для автоматической ротации | _(none)_ |
| `OLLAMA_API_KEY[1-N]` | Список ключей Ollama для автоматической ротации | _(none)_ |
| `NVIDIA_KEY[1-N]` | Список ключей Nvidia для автоматической ротации | _(none)_ |

---

## Ротация API Ключей (API Key Rotation)

Для обеспечения высокой отказоустойчивости, обхода жестких лимитов провайдеров (`HTTP 429 Too Many Requests`) и предотвращения простоев из-за исчерпания средств на балансе, сервер поддерживает **динамическую ротацию пулов API ключей** для всех основных провайдеров: **OpenRouter**, **Groq**, **Mistral**, **Ollama** и **Nvidia**.

### ⚙️ Как настроить ротацию
Вы можете прописать неограниченное количество ключей для любого провайдера в файле `.env` или передать их в качестве переменных окружения. Номерные ключи должны начинаться с `1` и идти последовательно:

```env
# Ключи для OpenRouter (используются в Ultraplinian Race и Model Discovery)
OPENROUTER_API_KEY=sk-or-v1-primary...
OPENROUTER_API_KEY1=sk-or-v1-first...
OPENROUTER_API_KEY2=sk-or-v1-second...
OPENROUTER_API_KEY3=sk-or-v1-third...

# Ключи для Mistral (прямые запросы)
MISTRAL_API_KEY1=6UBrR2T0P79nkkrCay3MNfuj3lmHR83T
MISTRAL_API_KEY2=8rxR7VdecIySJbhZMKybZrzm9Rp6YEbn

# Ключи для Groq (прямые запросы)
GROQ_API_KEY=gsk_primary...
GROQ_API_KEY1=gsk_first...
GROQ_API_KEY2=gsk_second...
```

### 🧠 Алгоритм работы ротации на сервере
1. **Случайный стартовый выбор (Traffic Balancing)**:
   При старте каждого запроса к модели сервер собирает все валидные ключи для данного провайдера и выбирает случайный стартовый индекс. Это позволяет равномерно распределять нагрузку по всем ключам.
2. **Мгновенный переход при ошибках (Failover Rotation)**:
   Если запрос завершается неудачей из-за:
   * Превышения лимитов (`HTTP 429` / Rate Limit)
   * Блокировки или исчерпания баланса (`HTTP 402` / `HTTP 403` / `Provider returned error` / `Quota exceeded`)
   * Любых временных сетевых ошибок и таймаутов
   
   Сервер **не прекращает работу**, а автоматически выбирает **следующий по порядку ключ в списке** и мгновенно производит повторную попытку (вплоть до 3 попыток на один запрос к модели).
3. **Логирование и прозрачность**:
   Каждый запрос подробно логируется в консоли сервера с маскированием ключей безопасности (отображаются только последние 4 символа), чтобы вы всегда могли отслеживать, какой именно ключ выполнил запрос или подвергся ротации:
   `[Ultraplinian] Querying groq/llama-3.3-70b-versatile (attempt 1/3) using key ...GnoR`
4. **Ротация в Model Discovery**:
   Фоновая служба периодического поиска бесплатных моделей также полностью поддерживает ротацию ключей OpenRouter. Если один из ключей временно заблокирован или перегружен, процесс сканирования бесплатных моделей плавно продолжит работу на резервных ключах.

### 🐳 Использование в Docker-контейнере
При запуске сервера через `./start.sh` скрипт автоматически анализирует ваши локальные переменные окружения (включая загруженные из `.env`), находит все ключи, подходящие под маски ротации, и динамически пробрасывает их внутрь Docker-контейнера. Вам больше не нужно вручную прописывать каждый ключ в аргументах запуска Docker!
